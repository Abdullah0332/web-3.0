    ...
    return (
        <nav className="w-full flex md:justify-center justify-between item-center p-4">
            ...
            <div className="flex relative">
                {toggleMenu 
                    ? 
                    <AiOutlineClose fontSize={48} className="text-white md:hidden cursor-pointer" onClick={() => setToggleMenu(false)}/> 
                    : 
                    < HiMenuAlt4 fontSize={48} className="text-white md:hidden cursor-pointer" onClick={() => setToggleMenu(true)}/>
                }
                {toggleMenu && (
                    <ul className="z-10 fixed top-0 -right-2 p-3 w-[70vw] h-screen shadow-2xl md:hidden list-none flex flex-col justify-start items-end rounded-md blue-glassmorphism text-white animated-slide-in" >
